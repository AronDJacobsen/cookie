...
def process_and_save_data(data_folder, output_folder):
    ...
    transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])  # assuming 0,1 range

    # Load the raw data
    raw_trainset = datasets.FashionMNIST(
        os.path.join(data_folder, "raw"), download=True, train=True, transform=transform
    )
    raw_testset = datasets.FashionMNIST(
        os.path.join(data_folder, "raw"), download=True, train=False, transform=transform
    )
