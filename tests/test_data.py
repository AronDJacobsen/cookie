...
processed_data_path = (
    _PATH_DATA + "/processed/processed_data.pt"
)  # Replace with the actual path to your processed data file
...
def test_data():
    ...
    for data in trainloader.dataset:
        assert data[0].shape == torch.Size([1, 28, 28]) or data[0].shape == torch.Size(
            [784]
        ), "The shape of the training set is incorrect"

    # Check the shape of each datapoint in the validation set
    for data in valloader.dataset:
        assert data[0].shape == torch.Size([1, 28, 28]) or data[0].shape == torch.Size(
            [784]
        ), "The shape of the validation set is incorrect"
